...
import 'stickybits';
  ...
  var Unifato = {
    ...
    enableSidebar: function() {
      ...
      try {
        Array.from(sidebars).map(function(sidebar) {
          ...
          Array.from(closeBtns).map(function(closeBtn) {
          ...
          Array.from(menus).map(function(menu){
            var menusWithSubmenu = menu.getElementsByClassName('menu-item-has-children');
            Array.from(menusWithSubmenu).map(function(menuWithSubmenu) {
    ...
    enableBlogSocialWidget: function() {
      ...
      var defaults = {useStickyClasses: true};
      Array.from(stickys).forEach(function(sticky) {
        var dataOptions = sticky.dataset.pluginOptions !== undefined ? JSON.parse(sticky.dataset.pluginOptions) : {};
        var options = Object.assign(defaults, dataOptions);
        var sticky = stickybits(sticky, options);
      });
    },
    ...
    enableCountup: function() {
      ...
      Array.from(elms).forEach(function(elm) {
        ...
        // options.end is our non-standard addition which we require to be set in data-plugin-options
        // we mixin data-plugin-options over defualts so consumers can overide any setting.
        var dataOptions = JSON.parse(elm.dataset.pluginOptions);
        var options = Object.assign(defaults, dataOptions);
        var instance = new CountUp(elm, options.end, options);
    ...
    enableShopSingleTabs: function() {
      try {
        ...
        Array.from(wooTabs).forEach(function(wooTab) {
